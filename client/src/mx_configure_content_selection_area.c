...
void mx_configure_content_selection_area(GtkWidget **content_selection_area, GtkWidget **main_area, 
    ...
    *content_selection_area = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_fixed_put(GTK_FIXED(*main_area), *content_selection_area, 0, 40);
    ...
    *more = gtk_drawing_area_new();
    g_signal_connect(G_OBJECT(*more), "draw",
                     G_CALLBACK(mx_draw_event_more), NULL);
    gtk_box_pack_start(GTK_BOX(*content_selection_area), *more, TRUE, FALSE, 22);
    gtk_widget_set_size_request(GTK_WIDGET(*more), 24, 24);
}
