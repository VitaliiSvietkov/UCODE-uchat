...
void mx_attach_send_message_on_enter(GtkWidget *widget, void **arr) {
    ...
    if (gdk_pixbuf_get_width(GDK_PIXBUF(pixbuf)) > 350) {
        ...
        sprintf(sql,
                "INSERT INTO Messages (id, gid, uid) VALUES('%u','%s','%u');",
                msg->id, curr_gid, msg->uid);
        ...
        if (text != NULL) {
            ...
            sprintf(sql,
                    "INSERT INTO Messages (id, gid, uid, Text) VALUES('%u','%s','%u','%s');",
                    msg->id, curr_gid, msg->uid, msg->text);
            sqlite3_exec(db, sql, 0, 0, &err_msg);
        }
    }
    else {
...
void mx_send_message_on_enter(GtkWidget *widget) {
    if (mx_strlen(gtk_entry_get_text(GTK_ENTRY(widget))) > 0) {
        ...
        char sql[500];
        bzero(sql, 500);
        sprintf(sql, "INSERT INTO Messages (id, gid, uid, Text) VALUES('%u','%s','%u','%s');", 
                msg->id, curr_gid, msg->uid, msg->text);
