...
void mx_create_edit_user_form() {
    ...
    GtkWidget *username_eventbox = gtk_event_box_new();
    gtk_widget_set_name(GTK_WIDGET(username_eventbox), "username_eventbox");
    gtk_widget_set_size_request(GTK_WIDGET(username_eventbox), 400, 40);
    gtk_box_pack_start(GTK_BOX(edit_user_form), username_eventbox, FALSE, FALSE, 15);
    GtkWidget *username_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_container_add(GTK_CONTAINER(username_eventbox), username_box);
    ...
    /*g_signal_connect(G_OBJECT(username_eventbox), "enter-notify-event",
        G_CALLBACK(username_eventbox_enter_notify_event), NULL);
    g_signal_connect(G_OBJECT(username_eventbox), "leave-notify-event",
        G_CALLBACK(username_eventbox_leave_notify_event), NULL);*/
    g_signal_connect(G_OBJECT(username_eventbox), "button_press_event",
        G_CALLBACK(username_eventbox_click_event), NULL);
}
